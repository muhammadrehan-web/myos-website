...
import { lerpKeys } from "@/lib/format";

export function IntroHello() {
  ...
  useEffect(() => {
    ...
    if (!intro || !p1 || !p2) return;
    let t0: number | null = null;

    const setPath = (el: SVGPathElement, dash: number, opacity: number) => {
    ...
    const frame = (now: number) => {
      ...
      if (!done.current && t >= 1980) dismiss();
      if (t >= 2550) {
        setGone(true);
        return;
      }
